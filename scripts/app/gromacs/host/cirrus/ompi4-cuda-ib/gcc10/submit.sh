...
module load openmpi/4.1.0-cuda-11.2
...
NCORESPN=`expr ${SLURM_CPUS_ON_NODE}`
NCORES=`expr ${NNODES} \* ${NCORESPN}`
NGPUSPN=<number of gpus per node>
NGPUS=`expr ${NNODES} \* ${NGPUSPN}`
...
srun -n ${NCORES} --distribution=block:block --hint=nomultithread --chdir=${APP_RUN_PATH} ${SINGULARITY_PATH} ${SINGULARITY_OPTS} ${CONTAINER_PATH} ${APP_EXE} ${APP_PARAMS} &>> ${APP_OUTPUT}
