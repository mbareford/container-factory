...
NEMO_BUILD_PATH=${NEMO_ROOT}/${NEMO_VERSION}/${HOST_PATH}
...
SCRIPTS_CFG_ROOT=/opt/scripts/app/${NEMO_LABEL}/cfg/${CFG_LABEL}
XIOS_INSTALL_ROOT=${XIOS_ROOT}/${XIOS_VERSION}/${HOST_PATH}
NEMO_INSTALL_ROOT=${CFG_PATH}
LOG_ROOT=/opt/logs
...
mkdir -p ${NEMO_BUILD_PATH}
cp -r ${NEMO_ROOT}/${NEMO_NAME}/* ${NEMO_BUILD_PATH}/
cd ${NEMO_BUILD_PATH}
...
# install
echo "" &>> ${MAKE_LOG}
echo "NEMO ${CFG_LABEL} input files written to ${NEMO_EXP00_PATH}/ ." &>> ${MAKE_LOG}
echo "NEMO executable written to ${NEMO_INSTALL_ROOT}/BLD/bin/nemo.exe ." &>> ${MAKE_LOG}
